...
*Figure 1: Architecture of the implemented pick-and-place program. Note that the control is open-loop, the feedback signal is derived exclusively from the forward model's estimated end-effector position, rather than from any physically measured state.*
...
While the benchmark handles the internal states (motor effort, TCP error), the physical success of the grasp should also be recorded. This evaluates the real-world performance of the end-effector itself. 
